...
class RuleCommand {
    constructor(logger) {
        this.buffer = []; // Mémorise les messages pour éviter les doublons
        this.logger = logger;
    }
    support(reactionName) {
    ...
    run(reaction, user) {
        ...
        if (message) {
            ...
            this.logger.log(`<@!${reaction.users.cache.first().id}> a utilisé le bot pour la règle ${reaction.emoji.name} sur le message ${permalink}`);
            reaction.message.reply(`:robot: **règle ${reaction.emoji.name}** : ${message}`);
...
//# sourceMappingURL=RuleCommand.js.map